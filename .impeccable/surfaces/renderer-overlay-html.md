...
**Amended 2026-07-29 (user request — "I wouldn't want to keep things that are not functional or useful"):** the two turning reels and the record-head marker (slit + aluminum shoe) are removed from the transport. They were the parts of the metaphor that reported nothing: the lamp block, the written tape and the counter already carry every state the reels gestured at, and the reels were spending ~92px of a 500px overlay to do it. The tape window absorbs that width (140px → ~216px) and the signal is now written flush at its right edge, carried left — so the well shows ~7s of the take's history edge to edge instead of reserving its left 40% as blank run-in (`TAPE_HEAD_X` is gone). The direction is unchanged: still The Tape Transport, now with no part on the faceplate that doesn't report state. The wizard's miniature deck follows suit (`renderer-settings-html.md`).
