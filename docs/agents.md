...
| Symptom | Fix |
| --- | --- |
| Nothing pastes | Check Settings → General isn't set to clipboard-only; on Linux install `xdotool`/`wtype`; on macOS grant Accessibility (Settings → Advanced → **Fix auto-paste permission**) and Automation (Privacy & Security → Automation → Earheart → System Events); the overlay and `earheart.log` name which one blocked the paste. |
